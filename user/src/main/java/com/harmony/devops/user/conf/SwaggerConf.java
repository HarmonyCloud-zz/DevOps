...
public class SwaggerConf {
    ...
    public Docket outApi() {
        return new Docket(DocumentationType.SWAGGER_2).groupName("outApi")// 定义组
                .select() // 选择那些路径和 api 会生成 document
                .apis(RequestHandlerSelectors.basePackage("com.harmony.devops.thirdplatform.api.app.v01")) // 拦截的包路径
                .build() // 创建
                .apiInfo(apiInfo()); // 配置说明
    }
    ...
    public Docket mobileApi() {
        return new Docket(DocumentationType.SWAGGER_2).groupName("mobileApi")// 定义组
                .select() // 选择那些路径和 api 会生成 document
                .apis(RequestHandlerSelectors.basePackage("com.harmony.devops.thirdplatform.api.app.v02")) // 拦截的包路径
                .build() // 创建
                .apiInfo(apiInfo()); // 配置说明
    }
