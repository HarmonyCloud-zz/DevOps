...
public class ConsumerUserServiceImpl implements ConsumerUserService {
    ...
    public ConsumerUserVO getConsumerUserByLoginNameAndPwd(String loginName, String passwd) {
        return dao.get("select new com.zhengtou.cf.user.pojo.vo.ConsumerUserVO(u.id,u.userNo,u.userType,u.lastLoginTime,u.personal.name,u.phone,u" +
    ...
    public List<DeviceVO> getDeviceByUserId(long userId) {
        return dao.find("select new com.zhengtou.cf.user.pojo.vo.person.DeviceVO(p.id,p.carrierType,p.deviceType,p.deviceIp,p.location,p" +
    ...
    public List<HouseVO> getHouseByUserId(long userId) {
        return dao.find("select new com.zhengtou.cf.user.pojo.vo.person.HouseVO(c.id,c.propertyNo,c.address,c.propertyType,c.plannedUse,c.area,c" +
    ...
    public List<CarVO> getCarByUserId(long userId) {
        return dao.find("select new com.zhengtou.cf.user.pojo.vo.person.CarVO(c.id,c.vehicleCompany,c.vehicleBrand,c.vehicleModel,c.vehiclePrice,c" +
    ...
    public PersonalVO getPersonalById(long id) {
        return dao.get("select new com.zhengtou.cf.user.pojo.vo.person.PersonalVO(p.id,p.name,p.gender,p.age,p.idType,p.idNo,p.cellphone,p" +
    ...
    public List<ContactVO> getContactByPersonalId(long personalId) {
        return dao.find("select new com.zhengtou.cf.user.pojo.vo.person.ContactVO(c.id,c.contactName,c.contactRelation,c.contactMobile) from " +
    ...
    public List<EducationalVO> getEducationalByPersonalId(long personalId) {
        return dao.find("select new com.zhengtou.cf.user.pojo.vo.person.EducationalVO(p.id,p.qualification,p.schoolLength,p.schoolEducationType,p" +
    ...
    public List<EmpVO> getEmpByPersonalId(long personalId) {
        return dao.find("select new com.zhengtou.cf.user.pojo.vo.person.EmpVO(p.id,p.empStandFrom,p.empYears,p.empUnitName,p.empWorkStandFrom,p" +
    ...
    public IncomeVO getIncomeByPersonalId(long personalId) {
        return dao.get("select new com.zhengtou.cf.user.pojo.vo.person.IncomeVO(p.id,p.monthIncome,p.otherIncome,p.yearIncome,p.otherLoan) from " +
    ...
    public List<OtherVO> getOtherByPersonalId(long personalId) {
        return dao.find("select new com.zhengtou.cf.user.pojo.vo.person.OtherVO(p.id,p.telcheckFlag,p.zhimaScore,p.applyAdressState,p" +
                ".applyAdressCity) " +
    ...
    public List<ConsumerUserVO> getConsumerUserList(QueryConsumerUserReciveVO consumerUserReciveVO, Integer page, Integer size) {
        StringBuffer hql = new StringBuffer("select new com.zhengtou.cf.user.pojo.vo.ConsumerUserVO(u.id,u.userNo,u.lastLoginTime,u.personal.name,u" +
    ...
    @Override
    public List<AnnexVO> findAnnexVOList(long userId, AnnexTypeEnum annexTypeEnum) {
        StringBuffer hql = new StringBuffer("select new com.zhengtou.cf.user.pojo.vo.AnnexVO(a.annexTypeEnum,a.url) from AnnexEntity a where a.user" +
                ".id=:userId and a.annexStatus=:annexStatus ");
        HashMap<String, Object> where = new HashMap<>();
        where.put("userId", userId);
        where.put("annexStatus", AnnexStatusEnum.启用);
        if (annexTypeEnum != null) {
            hql.append(" and a.annexTypeEnum=:annexTypeEnum");
            where.put("annexTypeEnum", annexTypeEnum);
        }
